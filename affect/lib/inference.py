...
def infer(context_norm, body_norm, ind2cat, ind2vad, device, thresholds, models, image_context_path=None, image_context=None, image_body=None, bbox=None, to_print=True):
  ...
  for i in range(len(bool_cat_pred)):
    if bool_cat_pred[i] == True:
      ...
      emotions_scores.append(pred_cat[i])

  sorted, indices = torch.sort(torch.tensor(emotions_scores))
  # print('sorted_idx: ', indices)
  # print(cat_emotions)

  sorted_emotions = [''] * len(indices)
  for i, emo in enumerate(cat_emotions):
    sorted_emotions[indices[i]] = emo
  # print('sorted emo: ', sorted_emotions)
  ...
  return sorted_emotions, 10*pred_cont
